...
TEST(WavefrontRendererTest, MaterialAwarePolicyProducesSameValueRange)
{
    ...
    WavefrontRenderer baselineRenderer(256, 4, SchedulingPolicy::None);
    baselineRenderer.renderScene(scene, camera, imageBaseline);

    WavefrontRenderer materialRenderer(256, 4, SchedulingPolicy::MaterialAware);
    ...
    EXPECT_NEAR(baselineBrightness, materialAwareBrightness, baselineBrightness * 0.15f);
}
